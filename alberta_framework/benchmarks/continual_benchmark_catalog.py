"""Discovery catalog and local-readiness checks for ASI comparison benchmarks.
...
role, and local readiness remain machine-readable. External qualification gates
and source pins remain authoritative in ``external_qualification.py``; this
catalog must not turn a supplied-array adapter into a qualified external suite.
"""
...
_SPECS: tuple[BenchmarkSpec, ...] = (
    ...
    BenchmarkSpec(
        ...
        setup_notes=(
            "A bounded supplied-array linear adapter exists, but it has no canonical loader, "
            "dataset checksum, held-out evaluation matrix, or Avalanche metric parity.",
        ),
    ),
    BenchmarkSpec(
        ...
        primary_metrics=("prequential accuracy", "average accuracy", "forgetting"),
        setup_notes=(
            "A bounded supplied-array linear adapter exists, but exact Avalanche transforms, "
            "dataset checksums, and held-out evaluation metrics remain open.",
        ),
    ),
    BenchmarkSpec(
        ...
        setup_notes=(
            "A bounded supplied-array linear adapter exists without Torch; exact Avalanche "
            "loading, normalization, augmentation, checksums, and metrics remain open.",
        ),
    ),
    BenchmarkSpec(
