...
OUTCOME_SCOPE = "official_cora_procgen_runtime_fixed_action_smoke_only"
...
def _require_authoritative_plan() -> None:
    literal_constants = (
        ...
        "official_cora_procgen_runtime_fixed_action_smoke_only",
        "provider_reported_unattested_external_runtime_numeric_arrays_nbytes_sum",
        (
            "LICENSE",
            "README.md",
...
class CORAProcgenSmokeReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.external_runtime_executed is not True
        ):
            raise ValueError("external smoke execution/mechanism/timing policy drifted")
        ...
        if self.outcome_scope != OUTCOME_SCOPE:
            raise ValueError("outcome scope exceeds the official-runtime smoke")
        if (
...
def build_smoke_receipt(
    ...
    *,
    ...
    """Copy and bind one trace supplied by the isolated official provider."""
