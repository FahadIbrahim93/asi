...
_MIN_CASES: Final[int] = 3
_MAX_CASES: Final[int] = 16
...
def _preflight_cases(
    ...
    if type(cases) is not tuple or not _MIN_CASES <= tuple.__len__(cases) <= _MAX_CASES:
        raise ValueError("panel must contain between 3 and 16 exact cases")
    ...
    for index, value in enumerate(resolved):
        ...
        checkpoint_id = _bounded_text(
            case.checkpoint_id, name=f"cases[{index}].checkpoint_id"
        )
...
def run_optimization_readiness_panel(
    ...
    readiness = [_internal_float(metric["optimization_readiness"]) for metric in metrics]
    ...
    for horizon in _HORIZONS:
        ...
        correlations[horizon] = _spearman(readiness, gains)
    finite_correlations = [value for value in correlations.values() if value is not None]
    if len(finite_correlations) != len(_HORIZONS) or any(
        value == 0.0 for value in finite_correlations
    ):
        outcome = "inconclusive"
    elif all(value > 0.0 for value in finite_correlations):
        outcome = "supported"
    else:
        outcome = "rejected"
    resources = {
        ...
        "data_steps": sum(
            _internal_int(resource["data_steps"]) for resource in child_resources
        ),
        "environment_steps": sum(
            _internal_int(resource["environment_steps"])
        ...
        "summed_child_preflight_work_units": sum(
            _internal_int(resource["preflight_work_units"])
            for resource in child_resources
        ),
        ...
        "max_child_peak_working_set_bytes": max(
            _internal_int(resource["peak_working_set_bytes"])
            for resource in child_resources
        ),
    ...
    payload: dict[str, object] = {
        ...
        "association": {
            ...
            "predictor": "optimization_readiness",
            "target": "future_relative_loss_reduction",
            "spearman_by_gain_horizon": correlations,
            "status": outcome,
