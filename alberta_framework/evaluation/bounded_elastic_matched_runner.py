...
DEVELOPMENT_SEEDS: Final = (0, 1, 2, 3, 4)
ARMS: Final = (
...
def run_bounded_elastic_matched(
    ...
    """Execute every registered structure arm for every frozen development seed."""
...
def validate_bounded_elastic_matched(
    ...
    _validate_bounded_elastic_matched(value, data_x, data_y, config=config, reexecute=True)
...
def _validate_bounded_elastic_matched(
    ...
    *,
    ...
    identities = {
        seed: _execution_identity(seed, checked_config, x.shape[0]) for seed in DEVELOPMENT_SEEDS
    }
    for row in rows:
...
def write_bounded_elastic_matched(
    ...
    *,
    ...
    """Validate and publish once, retaining negative outcomes by construction."""
    if type(destination) is not type(Path()):
        raise TypeError("destination must be an exact Path")
    validate_bounded_elastic_matched(value, data_x, data_y, config=config)
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = IPMNISTConfig(n_tasks=args.tasks, task_length=5000)
    x, y = load_mnist_train(args.data_home)
    result = run_bounded_elastic_matched(x, y, config=config)
    write_bounded_elastic_matched(args.output, result, x, y, config=config)
    return 0
