...
def test_campaign_runs_all_four_arms_across_frozen_seeds(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = cast(dict[str, Any], runner.run_bounded_elastic_matched(*_data(), config=SMALL))
    runner.validate_bounded_elastic_matched(result, *_data(), config=SMALL)
...
def test_validator_rejects_identity_resource_and_roster_forgery(
    ...
    with pytest.raises(ValueError, match="execution identity"):
        runner.validate_bounded_elastic_matched(forged, *_data(), config=SMALL)
    ...
    with pytest.raises(ValueError, match="roster"):
        runner.validate_bounded_elastic_matched(missing, *_data(), config=SMALL)
...
def test_writer_is_create_only_and_retains_negative_outcomes(
    ...
    monkeypatch.setattr(runner, "run_screening_config", _fake_run)
    result = runner.run_bounded_elastic_matched(*_data(), config=SMALL)
    destination = tmp_path / "bounded-elastic.json"
    runner.write_bounded_elastic_matched(destination, result, *_data(), config=SMALL)
    retained = json.loads(destination.read_bytes())
    runner.validate_bounded_elastic_matched(retained, *_data(), config=SMALL)
...
def test_preflight_rejects_unbounded_or_wrong_dataset_before_execution(
    ...
    with pytest.raises(ValueError, match="float32"):
        runner.run_bounded_elastic_matched(x.astype(np.float64), y, config=SMALL)
    huge = IPMNISTConfig(
        n_tasks=1,
        task_length=5000,
        input_dim=8_000,
    ...
    with pytest.raises(ValueError, match="persistent-memory"):
        runner.run_bounded_elastic_matched(x, y, config=huge)
    assert calls == 0
