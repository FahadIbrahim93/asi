"""Parity coverage for the issue #1566 low-cost activation and feature controls.

Existing coverage in `test_plasticity_comparators.py` exercises only the
mechanism-off reductions. These assertions bind each primitive to a property its
paper actually states, so a future edit that silently changes the mechanism
fails here rather than only shifting a benchmark number.
