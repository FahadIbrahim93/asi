...
def _bounded_campaign_dependencies(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def execute(
        ...
        **_kwargs: object,
        ...
        accuracy = 0.3 + 0.01 * arm_index + 0.001 * seed
...
def test_seed_identity_is_ambient_prng_invariant() -> None:
    ...
    try:
        ...
        threefry = campaign_module._seed_identity(0, config=_config(), n_train=4)
        jax.config.update("jax_default_prng_impl", "rbg")
        rbg = campaign_module._seed_identity(0, config=_config(), n_train=4)
    finally:
...
def test_default_plan_is_bounded_without_execution(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert config.n_steps * len(DEVELOPMENT_SEEDS) * len(registered_arms()) == 40_000_000
