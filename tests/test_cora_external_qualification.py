...
def test_external_provider_trace_round_trips_with_exact_receipts(
    ...
    assert checked.provider_trace_supplied_by_caller is True
    assert checked.external_runtime_execution_verified is False
...
def test_caller_arrays_cannot_forge_an_external_execution_claim(
    plan: CORAProcgenSmokePlan,
) -> None:
    receipt = _trace(plan)
    assert receipt.provider_trace_supplied_by_caller is True
    assert receipt.external_runtime_execution_verified is False
    forged = receipt.payload()
    forged["external_runtime_execution_verified"] = True
    with pytest.raises(ValueError, match="caller-supplied trace"):
