...
| Prediction, memory, and world modeling | Horde/GVF learners, working and experiential memory, one-step/latent/recurrent models, dreaming, and an L0 bounded recurrent sequence-replay → latent-imagination → lambda-return actor/value development consumer | No reference agent or promoted matched result shows that these components causally improve retained control under one owner and budget; the native sequence consumer is not DreamerV3/WMAR/Continual-Dreamer parity |
