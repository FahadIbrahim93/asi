...
| Agent composition | `PrototypeAgent`, explicit transition ownership, checkpoints, and Step/pipeline kernels | There is no selected reference configuration or uninterrupted L3 life; robot and Forager paths do not consume `PrototypeAgent`, whose closed-loop extended-action dispatch remains unresolved |
