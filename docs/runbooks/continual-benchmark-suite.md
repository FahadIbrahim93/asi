...
`list` is a discovery index of comparison protocols and audited upstream revisions. The
fail-closed external setup authority is
`alberta_framework/benchmarks/external_qualification.py`; where entries overlap, its gates and
pins control qualification. `doctor` performs only
...
- **Runnable native:** reference-life; IPMNIST when research dependencies and MNIST data are
  available; bounded Split/Rotated MNIST, Split CIFAR-100, and IPMNIST held-out qualification
  after `asi-native-supervised-canonical` accepts caller-held official release bytes.
- **Native external parity still open:** the supervised v3 gate qualifies official release
  checksums, exact loaders, held-out matrices, and named metric definitions. It does not qualify
  Avalanche transform/augmentation execution, external metric implementation parity, competitive
  deep baselines, or the full IPMNIST horizon.
- **Runnable optional:** Forager after installing the `forager` extra and satisfying its locks.
- **Scaffolded or qualification-only:** loss-of-plasticity, Continual World, CORA, COOM, and
  DreamerV3. Their bounded local smoke/adapter work does not make the external benchmark assets,
  runtimes, or paper protocols runnable or comparable.
- **Dataset qualification pending:** CLEAR10.

Use the corresponding GitHub issue before implementing an adapter. Passing every authoritative
qualification gate and landing a reviewed runner may move a suite from scaffolded to integrated;
a smoke trace alone does not. Integration does not make a result promoted evidence.
