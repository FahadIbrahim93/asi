...
ASI currently requires Python 3.12 or newer. The public package-index release under the legacy
distribution name does not track this fork's version, Python floor, or dependency extras, so a
bare `pip install alberta-framework` does not install this checkout. Install from a checkout
instead:
